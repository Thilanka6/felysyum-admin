...
export default function Sidebar({ isOpen, isCollapsed, setIsCollapsed, onCloseMobile }: SidebarProps) {
    const pathname = usePathname();
    const [isStakeFelyOpen, setIsStakeFelyOpen] = useState(false);

    // Auto-expand submenu if active
    useEffect(() => {
        if (pathname.includes('/approve-spending') || pathname.includes('/approved-spending')) {
            setIsStakeFelyOpen(true);
        }
    }, [pathname]);

    const isActive = (path: string) => pathname === path;
    const isStakeFelyActive = pathname.includes('/approve-spending') || pathname.includes('/approved-spending');
        ...
        ${isCollapsed ? 'md:w-20' : 'md:w-64'} w-64`}
            >
                <div className="flex h-16 items-center justify-center border-b border-gray-800 px-6">
                    {/* Large Logo */}
                    <img src="/assets/images/mat-logo.png" alt="Feliciamatrix"
                        className={`h-8 w-auto transition-all duration-300 ${isCollapsed ? 'hidden' : 'block'}`}
                    />
                    {/* Small Logo */}
                    <img src="/assets/images/mat-logo-s.png" alt="Felysyum"
                ...
                <nav className="mt-6 px-4 space-y-2">
                    {/* Main Section */}
                    <div className={`px-2 text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2 transition-opacity duration-300 ${isCollapsed ? 'hidden' : 'block'}`}>
                        Main
                    </div>
                    ...
                    >
                        ...
                        <span className={`whitespace-nowrap ${isCollapsed ? 'hidden' : 'block'}`}>Dashboard</span>
                    </Link>
                    ...
                    >
                        <div className="flex items-center gap-3">
                            ...
                            <span className={`whitespace-nowrap ${isCollapsed ? 'hidden' : 'block'}`}>Stake FELY</span>
                        </div>
                        <svg className={`h-4 w-4 bg-transparent transition-transform duration-200 ${isStakeFelyOpen ? 'rotate-180' : ''} ${isCollapsed ? 'hidden' : 'block'}`}
                            fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7" />
                        </svg>
                    </button>

                    <div className={`space-y-1 pl-11 ${isStakeFelyOpen ? 'block' : 'hidden'} ${isCollapsed ? 'hidden' : 'block'}`}>
                        <Link href="/approve-spending"
                    ...
                    {/* System Section */}
                    <div className={`px-2 text-xs font-semibold text-gray-400 uppercase tracking-wider mt-8 mb-2 transition-opacity duration-300 ${isCollapsed ? 'hidden' : 'block'}`}>
                        System
                    </div>
                    ...
                    >
                        ...
                        <span className={`whitespace-nowrap ${isCollapsed ? 'hidden' : 'block'}`}>Settings</span>
                    </Link>

                    <Link href="/login"
                        className="flex items-center gap-3 rounded-lg px-4 py-2.5 text-sm font-medium text-red-400 hover:bg-dark-800 hover:text-red-300 transition-all mt-4"
                        title="Logout"
                    >
                        <svg className="h-5 w-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2"
